...
def main():
    ...
    data=data.apply(lambda x: x.shift(1))
    data['diff']=data['diff'].shift(-1)
    data.loc[today.strftime('%Y-%m-%d'),'diff']=0
    ...
    history = model.fit(train_X, train_y, epochs=80, batch_size=100, validation_data=(test_X, test_y), verbose=2, shuffle=False)

    yhat = model.predict(pred_X)
    pred_y=inverse_scale(data['diff'], yhat )
    pred_y=pd.DataFrame(pred_y)
    pred_y.rename(columns={0:'diff'},inplace=True)
    pred_y.index = data.index[-1:]
    # mse=sqrt(mean_squared_error(test_y, yhat))

    pred_test_yhat=model.predict(test_X)
    pred_test_y=inverse_scale(data['diff'],pred_test_yhat )
    pred_test_y=pd.DataFrame(pred_test_y)
    pred_test_y.index = data.index[-len(test_y) - 1:-1]
    pred_test_y.rename(columns={0: 'diff'}, inplace=True)


    y_pred_recent=pred_test_y[-10:]
    y_pred_recent=pd.concat([y_pred_recent, pred_y])
    y_pred_recent.index=pd.to_datetime(y_pred_recent.index)



    plt.figure()
    plt.plot(data['diff'][-len(pred_test_y)-1:-1],label='actual')
    ...
    plt.figure()
    plt.plot(data['diff'][-11:-1],label='actual')
    plt.plot(y_pred_recent,label='pred')

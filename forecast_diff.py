...
timesteps=7
split_ratio=0.8
filename='/mnt/mfs/work_zhjing/train_data_300_500.csv'
...
def scale_data(data):
    scaler = MinMaxScaler()
    # data_X=data.drop('index_diff',axis=1)
    # data_Y=data['index_diff']
    data_transformed=scaler.fit_transform(data)
    # features_n = len(data.columns)
    # data_transformed=np.insert(data_X_transformed,features_n-1,data_Y.values,axis=1)
...
def main():
    ...
    plt.plot(model.predict(test_X),label='pred')
